...
class FragReturnReport extends StatelessWidget {
        ...
        children: [
          ...
          Obx(() => Container(
            ...
            child: DropdownButtonHideUnderline(
              child: DropdownButton<ReturnReportType>(
                value: controller.rxReportType.value,
                isDense: true,
                icon: Icon(Icons.keyboard_arrow_down_rounded, color: Colors.red.shade600),
                style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: Colors.grey.shade800),
                items: ReturnReportType.values.map((t) => DropdownMenuItem(
                  value: t,
                  child: Row(
                    mainAxisSize: MainAxisSize.min,
                    children: [
                      Icon(t.icon, size: 16, color: Colors.red.shade600),
  ...
  Widget _buildFilterBar(BuildContext context, ControllerReturnReport controller) {
    return Padding(
            ...
            child: Container(
              ...
              child: TextField(
                ...
                decoration: InputDecoration(
                  hintText: 'Search returns by invoice, customer, or supplier...',
  ...
  List<DataColumn> _columnsFor(ReturnReportType type, BuildContext context) {
    return switch (type) {
      ReturnReportType.salesReturn => [
        _col(context, 'Return Date', Icons.calendar_today_rounded, Colors.indigo),
        _col(context, 'Invoice No', Icons.receipt_long_rounded, Colors.orange),
        _col(context, 'Customer Name', Icons.people_outline, Colors.blue),
        _col(context, 'Total Items', Icons.unfold_more_rounded, Colors.orange),
        _col(context, 'Refunded Amount', Icons.monetization_on_rounded, Colors.green),
        _col(context, 'Reason / Notes', Icons.notes_rounded, Colors.grey),
      ],
      ReturnReportType.purchaseReturn => [
        _col(context, 'Return Date', Icons.calendar_today_rounded, Colors.indigo),
        _col(context, 'PO Number', Icons.local_shipping_outlined, Colors.orange),
        _col(context, 'Supplier Name', Icons.supervisor_account_rounded, Colors.blue),
        _col(context, 'Total Items', Icons.unfold_more_rounded, Colors.orange),
        _col(context, 'Refund Value', Icons.monetization_on_rounded, Colors.green),
        _col(context, 'Remarks / Notes', Icons.notes_rounded, Colors.grey),
      ],
    };
  }
    ...
    return switch (type) {
      ReturnReportType.salesReturn => rows.map((r) {
        ...
        return DataRow(cells: [
          ...
          DataCell(_badge(row.itemsCount.toString(), Colors.orange)),
          DataCell(Text(currFmt.format(row.refundAmount), style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13, color: Colors.red.shade700))),
          DataCell(Text(row.reason, style: TextStyle(fontSize: 12, color: Colors.grey.shade500))),
        ]);
      }).toList(),
      ...
      ReturnReportType.purchaseReturn => rows.map((r) {
        ...
        return DataRow(cells: [
          ...
          DataCell(Text(row.supplier, style: TextStyle(fontWeight: FontWeight.w600, fontSize: 13, color: Colors.grey.shade800))),
          DataCell(_badge(row.itemsCount.toString(), Colors.orange)),
          DataCell(Text(currFmt.format(row.refundAmount), style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13, color: Colors.red.shade700))),
          DataCell(Text(row.notes, style: TextStyle(fontSize: 12, color: Colors.grey.shade500))),
        ]);
      }).toList(),
    };
  }
        ...
        children: [
          Container(
            ...
            decoration: BoxDecoration(
              ...
              boxShadow: [
                BoxShadow(color: Colors.red.withOpacity(0.1), blurRadius: 20, offset: const Offset(0, 10)),
              ],
            ),
            child: Icon(Icons.assignment_return_rounded, size: 64, color: Colors.red.shade400),
          ),
          const SizedBox(height: 16),
          Text('No return records found', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w600, color: Colors.grey.shade600)),
  ...
  Widget _buildPagination(ControllerReturnReport controller) {
    return Container(
